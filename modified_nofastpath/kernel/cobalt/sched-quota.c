	...
	*/
	if (tg->nr_active == 0) {
		if(tg->run_budget_ns > 0) {
			...
			//printk(XENO_INFO "EGE\n");
		}
	}
	tg->nr_active++;
	//printk(XENO_INFO "E %d %d\n", thread->cprio, tg->nr_active);
}
	...
	*/ 
	if (tg->nr_active == 0) {
		if(!list_empty(&tg->quota_expired)) {
			list_del_init(&tg->quota_expired);
			//printk(XENO_INFO "DGE\n");
		} else {
			list_del(&tg->rlink);
			//printk(XENO_INFO "DG\n");
		}
	}
	//printk(XENO_INFO "D %d %d\n", thread->cprio, tg->nr_active);
}
...
static void xnsched_quota_requeue(struct xnthread *thread)
{
	...
	if (tg->nr_active == 0) {
		if(tg->run_budget_ns > 0) {
			...
			list_add_tail(&tg->quota_expired, &sched->quota.expired);
			//printk(XENO_INFO "RGE\n");
		}
	}
	tg->nr_active++;
	//printk(XENO_INFO "R %d %d\n", thread->cprio, tg->nr_active);
}
...
out:
	...
	//printk(XENO_INFO "S %d %d %lld\n", next->cprio, tg->nr_active, now);
	return next;
}
			     ...
			     int *quota_sum_r)
{
	...
	*/
	if (tg->run_budget_ns > 0 && tg->nr_active > 0) {
		if(!list_empty(&tg->quota_expired)) {
			list_del_init(&tg->quota_expired);
			list_add_priff(tg, &sched->quota.runnable, cprio, rlink);
		} else {
